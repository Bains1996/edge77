...
def _calculate_overcharge(fuel_surcharge: float, max_allowed: float) -> float:
    return max(0.0, fuel_surcharge - max_allowed)
...
def _build_dispute_email(audit_record: dict, contract: dict) -> tuple[str, str]:
    ...
    fuel_surcharge = audit_record.get("fuel_surcharge", 0.0)
    max_allowed = contract.get("max_allowed_fuel", 0.0)
    carrier_name = contract.get("carrier_name", "Carrier")
    carrier_billing_email = contract.get("carrier_billing_email", "")

    subject = (
        f"Fuel Surcharge Overcharge — Invoice {tracking_id} "
...
<table style="border-collapse: collapse; width: 100%; max-width: 500px;">
  <tr style="background: #f5f5f5;">
    <td style="padding: 8px; border: 1px solid #ddd;"><strong>Billed Fuel Surcharge</strong></td>
    <td style="padding: 8px; border: 1px solid #ddd;">{currency} {fuel_surcharge:.2f}</td>
  </tr>
  <tr>
    <td style="padding: 8px; border: 1px solid #ddd;"><strong>Contract Maximum Allowed</strong></td>
    <td style="padding: 8px; border: 1px solid #ddd;">{currency} {max_allowed:.2f}</td>
  </tr>
  <tr style="background: #fff3cd;">
    <td style="padding: 8px; border: 1px solid #ddd;"><strong>Overcharge Amount</strong></td>
    <td style="padding: 8px; border: 1px solid #ddd; color: #c0392b;"><strong>{currency} {overcharge:.2f}</strong></td>
  </tr>
  <tr style="background: #f5f5f5;">
    <td style="padding: 8px; border: 1px solid #ddd;"><strong>Audit Fee (15%)</strong></td>
...
def evaluate_and_dispute(extracted_json: dict, client_id: str) -> DisputeResult:
    ...
    max_allowed_fuel = contract.get("max_allowed_fuel", 0.0)
    ...
    overcharge = _calculate_overcharge(fuel_surcharge, max_allowed_fuel)

    if overcharge < min_overcharge:
        overcharge = 0.0

    fee = _calculate_fee(overcharge, fee_percentage) if overcharge > 0 else 0.0
    ...
    audit_data = {
        ...
        "fee_earned": fee,
        "status": status,
        "max_allowed_fuel": max_allowed_fuel,
        "dispute_mode": dispute_mode,
        "overcharge_detected": overcharge > 0,
    }
    ...
    if overcharge > 0:
        subject, body = _build_dispute_email(record, contract)
    ...
    logger.info(
        "[EDGE77 ENGINE] Evaluation complete: tracking=%s overcharge=%.2f fee=%.2f status=%s",
        tracking_id,
        overcharge,
        fee,
        status,
